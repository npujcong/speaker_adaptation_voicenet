...
if [ $stage -le 0 ]; then
  ...
  if $kaldi_format; then
    ...
    python ${voicenet_dir}/src/utils/convert_to_records_parallel.py --input_dim=297 --output_dim=199
  fi
fi
...
if [ $stage -le 1 ]; then
  ...
  echo "Training nnet"
  ./pyqueue_asr.pl --gpu 1 -l hostname=node7 log_file python $voicenet_dir/src/run_tts.py --save_dir=$dir "$@"
fi
