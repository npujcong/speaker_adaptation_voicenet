...
if [ $stage -le 3 ]; then
  ...
  python $voicenet_dir/misc/scripts/straight_mlpg/cmvn2dat.py \
       --var=$current_working_dir/var \
       --cmvn=$data/train_cmvn.npz
  python $voicenet_dir/misc/scripts/straight_mlpg/parameter_generation.py \
      --out_dir=$dir/orign \
      --var_dir=$current_working_dir/var/
  python $voicenet_dir/tools/LSP-VOCODER/gain_wav.py \
      --wav_dir=$dir/orign/wav \
      --mgc_dir=$dir/orign/mgc \
      --lf0_dir=$dir/orign/lf0 \
      --vocoder="lsp" \
      --use_lf0_mgc=True
fi
